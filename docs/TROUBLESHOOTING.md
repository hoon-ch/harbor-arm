...
1. **Missing generated configuration or environment**:

```bash
# Inspect the prepare-generated compose configuration for the service
docker compose -f docker-compose.yml config core
```

Harbor components require the configuration, secrets, certificates, and service
wiring generated by Harbor's `prepare` workflow. Do not treat a few environment
variables as a supported standalone deployment.

2. **Database not ready**:

```bash
# Check generated compose service health and dependencies
docker compose -f docker-compose.yml ps
docker compose -f docker-compose.yml logs --tail=100 postgresql core
```
...
# Or change the ports in the prepare-generated compose file and restart
docker compose -f docker-compose.yml config proxy
...
```bash
# Check generated compose networks
